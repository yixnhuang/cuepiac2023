...
def SoGetMainVehicleList(data):
	"""Get the main vehicle information list, you only need to get it once.
...
def SoAPISetFrameCB(startcb, stopcb):
	"""Callback for each frame in the simulation scene, the callback function is called at the beginning and end of each frame.
...
def SoGetMainVehicleStatus(mainVehicleId, data):
	"""Get the main vehicle status information.
...
def SoAPISetMainVehicleStatusUpdateCB(cb):
	"""Callback to obtain main vehicle status information.
